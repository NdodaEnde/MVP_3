    ...
    async (error: AxiosError): Promise<any> => {
      ...
      if (error.response?.status && [401, 403].includes(error.response.status) &&
        ...
        try {
          ...
          console.log("🔍 API DEBUG: Token refresh failed:", err);
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log("🔍 API DEBUG: Would redirect to login, but blocked for development");
            console.log("🔍 API DEBUG: Error details:", error.response?.status, error.response?.data);
            return Promise.reject(err);
          }
          
          // Only redirect in production
          console.log("🔍 API DEBUG: Redirecting to login (production mode)");
          window.location.href = '/login';
          return Promise.reject(err);
        }
      }

      // For development, log API errors but don't redirect
      if (process.env.NODE_ENV === 'development') {
        console.log("🔍 API DEBUG: API call failed:", {
