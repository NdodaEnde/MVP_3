...
import type { QuestionnaireFormData } from '@/schemas/questionnaireSchema';
...
export const StaffAdminInterface: React.FC = () => {
  // 📊 State Management
...
const PatientQueueItem: React.FC<{
  ...
  const getUrgencyLevel = (patient: Patient) => {
    if (patient.medicalAlerts && patient.medicalAlerts.length > 0) return 'high';
    if (patient.status === 'questionnaire') return 'medium';
    return 'low';
  };
  ...
  return (
    <div className={`p-4 rounded-lg border-2 hover:shadow-md transition-all ${
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <div className="relative">
            ...
            {patient.medicalAlerts && patient.medicalAlerts.length > 0 && (
              <div className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center">
                <span className="text-xs text-white font-bold">{patient.medicalAlerts.length}</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-1">
            ...
            {patient.questionnaireProgress !== undefined && (
              <div className="flex items-center space-x-2">
                <span className="text-xs text-gray-500">Progress:</span>
                <Progress value={patient.questionnaireProgress} className="w-24 h-2" />
                <span className="text-xs font-medium">{patient.questionnaireProgress}%</span>
              </div>
            )}
          </div>
        </div>
...
// 📋 Questionnaire Management Component
    ...
    <div className="space-y-6">
      ...
      {showValidationOverride && (
        <Card className="border-amber-200 bg-amber-50">
          <CardHeader>
            <CardTitle className="text-amber-800">Validation Override Required</CardTitle>
            <CardDescription className="text-amber-700">
              Please provide a reason for overriding the validation rules
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center space-x-2">
              <Button 
                onClick={() => onValidationOverride(overrideReason)}
              ...
              <Button 
                variant="outline" 
                onClick={() => setShowValidationOverride(false)}
              >
                Cancel
              </Button>
            </div>
